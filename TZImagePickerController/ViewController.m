...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _selectedPhotos.count) {
        ...
        if ([[asset valueForKey:@"filename"] containsString:@"GIF"] && self.allowPickingGifSwitch.isOn) {
            ...
            YZJMinePreviewPhotoViewController *previewVc = [[YZJMinePreviewPhotoViewController alloc] init];

            previewVc.photos = [NSMutableArray arrayWithArray:_selectedPhotos];
            previewVc.currentIndex = indexPath.row;
            previewVc.selectedAssets = _selectedAssets;
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
    ...
    YZJEditPhotoPreviewViewController* imagePicker = [[YZJEditPhotoPreviewViewController alloc] init];
    imagePicker.image = [photos firstObject];
    //                            TZImagePickerController *imagePicker = [[TZImagePickerController alloc] initCropTypeWithAsset:assetModel.asset photo:image completion:^(UIImage *cropImage, id asset) {
    //                                [self refreshCollectionViewWithAddedAsset:asset image:cropImage];
    //                            }];
    //                            imagePicker.needCircleCrop = self.needCircleCropSwitch.isOn;
    ...
    imagePicker.submitButtonClickBlockCropMode = ^(UIImage *cropedImage) {
        _selectedPhotos = [NSMutableArray arrayWithArray:photos];
        [_selectedPhotos replaceObjectAtIndex:0 withObject:cropedImage];
        _selectedAssets = [NSMutableArray arrayWithArray:assets];
        _isSelectOriginalPhoto = isSelectOriginalPhoto;
        [_collectionView reloadData];
        // _collectionView.contentSize = CGSizeMake(0, ((_selectedPhotos.count + 2) / 3 ) * (_margin + _itemWH));
